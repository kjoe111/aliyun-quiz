...
async function injectSyncFix(resp){
  ...
  const tag='<script src="./sync-fix.js?v=7"></script>';
...
self.addEventListener('activate',e=>e.waitUntil((async()=>{
  ...
  await Promise.all(keys.filter(k=>k!==CACHE&&k!=='aliyun-quiz-question-data-v1').map(k=>caches.delete(k)));
...
self.addEventListener('fetch',e=>{
  ...
  if(new URL(e.request.url).pathname.endsWith('/sync-fix.js')){
    e.respondWith(fetch(e.request,{cache:'no-store'}).then(resp=>{if(resp.ok){const copy=resp.clone();caches.open(CACHE).then(c=>c.put('./sync-fix.js',copy));}return resp;}).catch(()=>caches.match('./sync-fix.js')));
    return;
  }
